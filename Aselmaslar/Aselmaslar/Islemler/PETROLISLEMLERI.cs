...
namespace Aselmaslar.Islemler
{
    public class PETROLISLEMLERIIslem
    {
        ...
        {
            MySqlCommand cm = DBCon.KomutOlustur("insert into PETROLISLEMLERI(TARIH,FIS_NO,TUTAR,PETROL_ID,PLAKA_ID,TASYCFRM_ID,ACIKLAMA,CIKIS_ID)values (@TARIH,@FIS_NO,@TUTAR,@PETROL_ID,@PLAKA_ID,@TASYCFRM_ID,@ACIKLAMA,@CIKIS_ID)");
        ...
        {
            MySqlCommand cm = DBCon.KomutOlustur("update PETROLISLEMLERI set TARIH=@TARIH,FIS_NO=@FIS_NO,TUTAR=@TUTAR,PETROL_ID=@PETROL_ID,PLAKA_ID=@PLAKA_ID,TASYCFRM_ID=@TASYCFRM_ID,ACIKLAMA=@ACIKLAMA,CIKIS_ID=@CIKIS_ID where ID=@ID");
            cm.Parameters.AddWithValue("@TARIH", p.TARIH);
            cm.Parameters.AddWithValue("@FIS_NO", p.FIS_NO);
            cm.Parameters.AddWithValue("@TUTAR", p.TUTAR);
            cm.Parameters.AddWithValue("@PETROL_ID", p.PETROL_ID);
        ...
        {
            MySqlCommand cm = DBCon.KomutOlustur("delete from PETROLISLEMLERI where ID=@ID");
        ...
        {
            MySqlDataAdapter da = new MySqlDataAdapter("select * from PETROLISLEMLERI where ID=@ID", DBCon.BaglantiYap());
        ...
        {
            MySqlDataAdapter da = new MySqlDataAdapter("select * from PETROLISLEMLERI order by ID desc", DBCon.BaglantiYap());
            DataTable dt = new DataTable();
            da.Fill(dt);
            return dt;
        }
