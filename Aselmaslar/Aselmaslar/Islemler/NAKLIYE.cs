...
namespace Aselmaslar.Islemler
{
    public class NAKLIYEIslem
    {
        ...
        {
            MySqlCommand cm = DBCon.KomutOlustur("insert into NAKLIYE(ALICIFRM_ID,TASYCFRM_ID,GRUP_ID,URUN_ID,GONDRLNYER_ID,PLAKA_ID,TONAJ,TARIH,FISNO,IRSALIYENO,ACIKLAMA,CIKIS_ID)values (@ALICIFRM_ID,@TASYCFRM_ID,@GRUP_ID,@URUN_ID,@GONDRLNYER_ID,@PLAKA_ID,@TONAJ,@TARIH,@FISNO,@IRSALIYENO,@ACIKLAMA,@CIKIS_ID)");
        ...
        {
            MySqlCommand cm = DBCon.KomutOlustur("update NAKLIYE set ALICIFRM_ID=@ALICIFRM_ID,TASYCFRM_ID=@TASYCFRM_ID,GRUP_ID=@GRUP_ID,URUN_ID=@URUN_ID,GONDRLNYER_ID=@GONDRLNYER_ID,PLAKA_ID=@PLAKA_ID,TONAJ=@TONAJ,TARIH=@TARIH,FISNO=@FISNO,IRSALIYENO=@IRSALIYENO,ACIKLAMA=@ACIKLAMA,CIKIS_ID=@CIKIS_ID where ID=@ID");
            cm.Parameters.AddWithValue("@ALICIFRM_ID", p.ALICIFRM_ID);
            cm.Parameters.AddWithValue("@TASYCFRM_ID", p.TASYCFRM_ID);
            cm.Parameters.AddWithValue("@GRUP_ID", p.GRUP_ID);
            cm.Parameters.AddWithValue("@URUN_ID", p.URUN_ID);
        ...
        {
            MySqlCommand cm = DBCon.KomutOlustur("delete from NAKLIYE where ID=@ID");
        ...
        {
            MySqlDataAdapter da = new MySqlDataAdapter("select * from NAKLIYE where ID=@ID", DBCon.BaglantiYap());
        ...
        {
            MySqlDataAdapter da = new MySqlDataAdapter("select * from NAKLIYE", DBCon.BaglantiYap());
            DataTable dt = new DataTable();
            da.Fill(dt);
            return dt;
        }
